...
# New Poliigon simple and mosaic use Vector Rotate nodes, only avail in 2.9
if bpy.app.version >= (2, 90):
	MAPPING_ITEMS = (
		("uv_simple_mapping", "Poliigon Default - UV",
			"Use UV, Poliigon Simple mapping node group"),
		("uv_mosaic_mapping", "Mosaic De-tiling - UV",
			"Use UV, de-tiling Poliigon Mosaic mapping node group"),
		("uv_standard", "Blender Original - UV",
			"Use UV, built in mapping, flat projection"),
		("box_standard", "Box Mapping - Generated",
			"Use box coordinate type mapping, and built in mapping node"),
...
def folderset_list_update(context, items):
	...
	# Debug printing for detected thumbnails
	thumb_items = [itm[-1] for itm in items_sorted if itm[-1]]
	print_verbose("Loading {} materials, with {} thumbnails for {}".format(
		len(items_sorted), len(thumb_items), pmcprop.preview_type))
	if thumb_items:
		print_verbose("Example icon path: " + thumb_items[0])
	else:
		print_verbose("No matched icons to load!")
...
class PMC_load_and_apply_material(bpy.types.Operator):
	...
	def execute(self,context):
		...
		if context.selected_objects:
			...
			if pmcs.use_disp and pmcs.use_micro_displacements:
				self.report({"WARNING"}, "Default displacement = 0.05, consider adjusting")
			elif pmcs.use_disp:
...
class PMC_load_material_popup(bpy.types.Operator):
	...
	use_bump = bpy.props.BoolProperty(
		name="Include Bump maps",
		description="Use Bump maps (if available)",
		default=True
	)
	...
	def execute(self, context):
		...
		# check added to run from limited context
...
class PMC_apply_material(bpy.types.Operator):
	...
	def execute(self, context):
		...
		using_uv = pmcs.mapping_type in (
			'uv_simple_mapping', 'uv_mosaic_mapping', 'uv_standard')
		if count_no_UV > 0 and using_uv:
...
class PMC_add_converter_node(bpy.types.Operator):
	bl_idname = "pmc.add_converter_node"
	bl_label = "Converter node group"
	bl_description = "Adds a material converter node group"
	bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

	enum29 = (
		("pbr_mixer", "Principled mixer", "Principled mixer node"),
		("simple_uv_mapping", "Simple mapping", "Poliigon Simple mapping node"),
		("mosaic_uv_mapping", "Mosaic mapping", "Poliigon Mosaic mapping node"),
		("loaded_material", "Loaded material", "Load in a downloaded material"),
	)
	enum28 = (
		("pbr_mixer", "Principled mixer", "Principled mixer node"),
		("loaded_material", "Loaded material", "Load in a downloaded material"),
	...
	node_type = bpy.props.EnumProperty(
		items=enum29 if bpy.app.version >= (2, 90) else enum28)

	loaded_material = bpy.props.StringProperty(name="Custom material", default="")
	...
	def execute(self, context):
		...
		if self.node_type == "loaded_material" and not self.loaded_material:
			self.report({"ERROR"}, "Loaded material not found")
			return {"CANCELLED"}

		for node in context.material.node_tree.nodes:
			node.select = False

		self._workflow = PMC_workflow()
		self._workflow.material = context.material

		bpy.ops.node.add_node(type="ShaderNodeGroup", use_transform=True)
		node = context.material.node_tree.nodes.active
		node.node_tree = self.get_or_create_nodegroup()
		node.width = 200
	...
	def get_or_create_nodegroup(self):
		node_group = None
		if self.node_type == "loaded_material":
			for mat in bpy.data.materials:
				if mat.name != self.loaded_material:
					continue
				...
				for node in mat.node_tree.nodes:
					if node.type != 'GROUP':
						continue
					if not node.name.lower().startswith(self.loaded_material.lower()):
						continue
					node_group = node.node_tree
					break
				break
		else:
			mat_config = self._workflow.load_nodegroup_config(self.node_type)
			node_group = self._workflow.create_nodegroup_from_config(mat_config)
...
class PMC_scene_props(bpy.types.PropertyGroup):
	...
	use_bump = bpy.props.BoolProperty(
		name="Include Bump maps (if available)",
		description="Use Bump maps (if available)",
		default=True
	)
	use_sixteenbit = bpy.props.BoolProperty(
...
class PMC_matprops(bpy.types.PropertyGroup):
	...
	use_bump = bpy.props.BoolProperty(default=True)
