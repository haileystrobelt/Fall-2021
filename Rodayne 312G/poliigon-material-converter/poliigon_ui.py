...
def append_pmc_groups_node_add(self, context):
	"""Appending to add node menu, for Poliigon node groups"""
	self.layout.menu('PMC_MT_add_node_groups')


class PMC_MT_add_node_groups(bpy.types.Menu):
	"""Menu for the Poliigon Material Converter material groups"""
	bl_space_type = 'NODE_EDITOR'
	bl_label = "Poliigon group"

	def draw(self, context):
		layout = self.layout
		col = layout.column(align=True)
		if bpy.app.version >= (2, 90):
			col.operator("pmc.add_converter_node",
				text="Poliigon Simple").node_type = "simple_uv_mapping"
			col.operator("pmc.add_converter_node",
				text="Mosaic").node_type = "mosaic_uv_mapping"
		col.operator("pmc.add_converter_node",
			text="PBR mixer").node_type = "pbr_mixer"

		col.separator()
		converter_loaded = [mat for mat in bpy.data.materials
			if mat.pmc_matprops.workflow]  # ie non empty string
		if not converter_loaded:
			col.label(text="(No converter materials loaded)")
		for mat in converter_loaded:
			ops = col.operator("pmc.add_converter_node", text=mat.name)
			ops.node_type = "loaded_material"
			ops.loaded_material = mat.name

			# Blender says this property is read-only.
			# ops.settings = [{
			# 	"name": "node_tree",
			# 	"value": "bpy.data.node_groups['{}']".format(mat.name)
			# }]
...
def register():
	...
	# add to the NODE_add menu
	bpy.utils.register_class(PMC_MT_add_node_groups)
	bpy.types.NODE_MT_add.append(append_pmc_groups_node_add)
...
def unregister():
	...
	bpy.types.NODE_MT_add.remove(append_pmc_groups_node_add)
	bpy.utils.unregister_class(PMC_MT_add_node_groups)
