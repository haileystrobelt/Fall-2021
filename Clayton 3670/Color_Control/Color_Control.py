...
def colorChange(i, control):
    ...
    if i.isnumeric():  # if it can be converted to a number
        cmds.color(control, ud=int(i))  # use index, make sure it's converted though
    if i in colorList:
        i = colorList.index(i)  # if the index is in the list, find which index it's at
        cmds.color(control, ud=int(i))  # use that index to get the color in maya
    else:
